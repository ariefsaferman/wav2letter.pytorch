...
parser.add_argument('--save-folder', default='epoch/', help='Location to save epoch models')
parser.add_argument('--model-path', default='models_indo/wav2letter_indonesia.pth.tar',
...
if __name__ == '__main__':
    ...
    input_batch_len = args.batch_size
    num_batches = args.batch_size
    target_batch_len = 1

    criterion = ASGLoss(num_labels = num_labels, reduction='mean',  
                       gpu_no_stream_impl=True, 
                       forward_only=True)
    # criterion = ASGLoss()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            # Coba2 
            # input_batch_len = len(inputs) ==> out
            # num_batches = len(targets)
            # num_labels = len(target_sizes) 
            # target_batch_len = len(targets) 
            # print("INPUTS: " + str(inputs[0]))

            inputs = torch.randn(len(inputs), num_batches, num_labels, requires_grad=True)
            targets = torch.randint(0, num_labels, (num_batches, target_batch_len))
            input_lengths = torch.randint(1, len(inputs) + 1, (num_batches,))
        ...
        early_stopping.stopping(wer, model, optimizer=optimizer, epoch=epoch, loss_result=loss_results, wer_result=wer_results, cer_result=cer_results)
